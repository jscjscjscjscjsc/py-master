...
const Training = {
  ...
  async runCell(index) {
    ...
    const data = await this.api('/api/training/run-cell', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ cells, active: index, stdin: stdin ? stdin.value : '' }),
    });
  ...
  async submit() {
    ...
    if (data.passed) {
      ...
      const starText = '★'.repeat(settle.stars || 0) + '☆'.repeat(3 - (settle.stars || 0));
      this.panel('settle', '🎉 通过！', `
        <div class="settle-pop">
          <span class="big">${starText}</span>
          <div class="txt">本次掌握度 <b>${settle.stars || 0} 星</b>。
            ${settle.note ? this.esc(settle.note) : ''}
            ${settle.clears > 1 ? `这是第 ${settle.clears} 次通关，重复练习的分数会递减——想拿高分就一次做对。` : ''}
          </div>
