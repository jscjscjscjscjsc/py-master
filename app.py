...
@app.route('/')
def index():
    # 每次打开站点都先放一遍开场 CG，再进去登录 / 进平台。
    #
    # 原先是"看过就记 Cookie，之后直接跳过"。改掉的理由：
    #   · 公网版的入口是发给学生的，他第一次打开的那一下**必须**看到这支片子，
    #     而"换浏览器/清缓存/换设备"都会让 Cookie 失效或复现，行为飘忽不定；
    #   · 片头自带「跳过片头 ↗」，看过的人一秒就能过，重放的代价很低；
    #   · 一致比省事更重要 —— 每次进来都是同一个开场，不用猜这次会不会放。
    # 注意：只有站点根路径会播；直接点开某一章的链接（/chapter/5）不会重放。
